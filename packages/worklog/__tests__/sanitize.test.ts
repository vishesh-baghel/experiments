...
const baseSession: NormalizedSession = {
  ...
  turns: [
    ...
    {
      role: "assistant",
      content: "I found the issue in middleware.",
...
describe("sanitizeRuleBased", () => {
  describe("term redaction", () => {
    it("replaces terms in turn content (case-insensitive)", () => {
      const config = {
        ...baseConfig,
        redactedTerms: { "ubixi.com": "work" },
      };
      const session = {
        ...baseSession,
        turns: [
          {
            role: "user" as const,
            content: "Deploy to UBIXI.COM staging",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
      const result = sanitizeRuleBased(session, config);
      expect(result!.turns[0].content).toBe("Deploy to work staging");
    });

    it("replaces terms in session project name", () => {
      const config = {
        ...baseConfig,
        redactedTerms: { ubixi: "work" },
      };
      const session = {
        ...baseSession,
        project: "ubixi-dashboard",
      };
      const result = sanitizeRuleBased(session, config);
      expect(result!.project).toBe("work-dashboard");
    });

    it("replaces terms in session summary", () => {
      const config = {
        ...baseConfig,
        redactedTerms: { batonsystems: "work" },
      };
      const session = {
        ...baseSession,
        summary: "Fixed bug in batonsystems payment flow",
      };
      const result = sanitizeRuleBased(session, config);
      expect(result!.summary).toBe("Fixed bug in work payment flow");
    });

    it("replaces multiple terms", () => {
      const config = {
        ...baseConfig,
        redactedTerms: { "ubixi.com": "work", "batonsystems.com": "work" },
      };
      const session = {
        ...baseSession,
        turns: [
          {
            role: "user" as const,
            content:
              "Migrate from batonsystems.com to ubixi.com for the new API",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
      ...
      expect(result!.turns[0].content).toBe(
        "Migrate from work to work for the new API",
      );
    });
    ...
    it("leaves non-matching content unchanged", () => {
      ...
      expect(result!.turns[1].content).toBe(
        "I found the issue in middleware.",
      );
    });
    ...
    it("escapes regex special characters in terms", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Deploy to company.io",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
      const result = sanitizeRuleBased(session, config);
      expect(result!.turns[0].content).toBe("Deploy to work");
    });

    it("redacts terms before blocked-turn checks apply", () => {
      const config: SanitizationConfig = {
        blockedProjects: [],
        blockedPaths: [],
        blockedDomains: ["secret.internal.com"],
        redactedTerms: { "secret.internal.com": "work-api" },
      };
      const session = {
        ...baseSession,
        turns: [
          {
            role: "user" as const,
            content: "Call secret.internal.com/api/data",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
      const result = sanitizeRuleBased(session, config);
      // Term is redacted first, so blocked domain check no longer matches
      expect(result).not.toBeNull();
      expect(result!.turns[0].content).toBe("Call work-api/api/data");
    });
  });
    ...
    it("redacts API keys with colon separator", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Set api_key: sk_live_abc123def456ghi789",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("redacts API keys with equals separator", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "token=abcdefghijklmnopqrstuv12345",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
        ...
        turns: [
          {
            ...
            content:
              "Use Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0 for auth",
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
        ...
        turns: [
          {
            role: "user" as const,
            content:
              "Token is ghp_1234567890abcdefghijklmnopqrstuvwxyz",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
        ...
        turns: [
          {
            role: "assistant" as const,
            content:
              'JWT: eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4ifQ',
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
    ...
    it("redacts sk- prefixed keys", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "OPENAI: sk-proj-abcdefghijklmnopqrstuvwx",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("redacts IPv4 addresses", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Connect to 192.168.1.100 on port 3000",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("redacts internal/localhost URLs", () => {
      const session = {
        ...
        turns: [
          {
            role: "assistant" as const,
            content: "Server at http://localhost:3000/api/health",
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
    ...
    it("redacts 10.x.x.x IPs in URLs", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "API at http://10.0.1.50:8080/internal",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("redacts 172.16-31.x.x IPs in URLs", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Check http://172.20.5.10/dashboard",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("redacts 192.168.x.x IPs in URLs", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Visit http://192.168.0.1/admin",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
        ...
        turns: [
          {
            role: "user" as const,
            content:
              "api_key: abc123xyz token=def456uvw secret: ghi789rst",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("preserves non-secret content unchanged", () => {
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Implement the user authentication flow",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
  ...
  describe("blocked paths", () => {
    it("removes turns containing blocked paths", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Edit /home/user/work/secret.ts",
            timestamp: "2025-01-22T10:00:00Z",
          },
          {
            role: "assistant" as const,
    ...
    it("returns null when all turns are blocked", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Edit /home/user/work/file.ts",
            timestamp: "2025-01-22T10:00:00Z",
          },
          {
            role: "assistant" as const,
            content: "Found issue at /home/user/work/auth.ts",
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
    ...
    it("checks multiple blocked paths", () => {
      const config = {
        ...baseConfig,
        blockedPaths: ["/opt/internal", "/home/user/work"],
      };
      const session = {
        ...baseSession,
        turns: [
          {
            role: "user" as const,
            content: "Edit /opt/internal/config.yaml",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
    ...
    it("removes turns mentioning blocked projects", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Working on the secret-client dashboard",
            timestamp: "2025-01-22T10:00:00Z",
          },
          {
            role: "assistant" as const,
            content: "Here is the fix for portfolio",
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
    ...
    it("performs case-insensitive project blocking", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "The SECRET-CLIENT app needs fixing",
            timestamp: "2025-01-22T10:00:00Z",
          },
        ],
      };
  ...
  describe("blocked domains", () => {
    it("removes turns mentioning blocked domains", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Check internal.company.com/dashboard",
            timestamp: "2025-01-22T10:00:00Z",
          },
          {
            role: "assistant" as const,
            content: "The public API looks good",
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
    ...
    it("blocks multiple domains", () => {
      const config = {
        ...baseConfig,
        blockedDomains: ["jira.company.com", "confluence.company.com"],
      };
      const session = {
        ...baseSession,
        turns: [
          {
            role: "user" as const,
            content: "See jira.company.com/PROJ-123",
            timestamp: "2025-01-22T10:00:00Z",
          },
          {
            role: "assistant" as const,
            content: "Check confluence.company.com/wiki/page",
            timestamp: "2025-01-22T10:01:00Z",
          },
        ],
      };
  ...
  describe("combined filters", () => {
    it("applies redaction and blocking together", () => {
      ...
      const session = {
        ...
        turns: [
          {
            role: "user" as const,
            content: "Edit /opt/work/file.ts",
            timestamp: "2025-01-22T10:00:00Z",
          },
          {
            role: "assistant" as const,
            content:
              "Used api_key: sk_test_1234567890abcdef to auth",
            timestamp: "2025-01-22T10:01:00Z",
          },
          {
            role: "user" as const,
            content: "Check internal.co/page",
            timestamp: "2025-01-22T10:02:00Z",
          },
          {
            role: "assistant" as const,
            content: "The feature is ready",
    ...
    it("all sessions pass through when no allowlist exists", () => {
      const session = { ...baseSession, project: "random-work-project" };
      const result = sanitizeRuleBased(session, baseConfig);
      expect(result).not.toBeNull();
      expect(result!.project).toBe("random-work-project");
    });
  });
});
